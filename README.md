...
nixdocker' answer: run the exact same generator **inside the Nix build sandbox**, at build time, and
...
own test suite, and this exact technique is core mechanism for nixdocker, not a dependency this repo
...
and restart conventions -- is nixdocker' own, not vendored from anywhere.
...
The generator wrapper above is table stakes; the reason to reach for nixdocker instead of a
...
container's image with no corresponding Nix config change is precisely the drift nixdocker exists
...
`nixdocker.containers.<name>.rootless.uid` is `null` (rootful, the system-wide manager) unless set.
...
Build-time synthesis sidesteps this completely, root or rootless alike, because nixdocker never
...
- the target uid's `--user` manager instance needs `users.users.<name>.linger = true` to exist
  unprompted at boot -- nixdocker warns by name when it can't confirm this, but does not manage
...
```nix
nixdocker.containers.web.health.cmd = "curl -f http://localhost/health";
# interval/timeout/retries/startPeriod all have sane defaults; only `cmd` has none, on purpose --
# a healthcheck nixdocker invented on this service's behalf would be a fake signal.

nixdocker.containers.web.restart.policy = "on-failure"; # the default; always overridable
```
...
host; nixdocker asserts against that combination at build time instead. All three are transcripts
in [`docs/gotchas.md`](docs/gotchas.md), not assertions.

`nixdocker.ripper` is the first module built out of this shape -- see "Appliances" below.

## Two planes, one declaration

nixdocker runs on NixOS (`nixosModules.nixdocker`) and on a foreign distro through
[system-manager](https://github.com/numtide/system-manager)
(`systemManagerModules.nixdocker`). A container is declared the same way on either; there is no
...
`modules/nixdocker.nix` holds everything that is true on both planes. Each backend adds only what
cannot be shared:

| | NixOS (`modules/nixos.nix`) | system-manager (`modules/system-manager.nix`) |
|---|---|---|
| podman | `virtualisation.podman.enable`, and the generator defaults to that same package | the distro's own package; units are pointed at `nixdocker.podman.path` (`/usr/bin/podman`) |
...
The generator that runs at build time is always a Nix package (`nixdocker.podman.package`) -- there
is no other way to translate at build time. What `nixdocker.podman.path` changes is only the binary
...
`nixdocker.ripper` -- an optical-disc ripper (`rix1337/docker-ripper`) as an on-demand podman job
...
lives in `modules/ripper.nix`. It renders into `nixdocker.containers.<name>` like any other caller,
...
nixdocker is for workloads that are single-host BY NATURE -- real local state, a hardware
...
**vs. k3s** -- k3s is the default for services here. nixdocker is for workloads that are
...
"It's small" is not a reason to reach for nixdocker; "it cannot be anything but this one host" is.
...
question nixdocker already answers.
...
yourself). Reach for nixdocker specifically when pod-grouping or a typed healthcheck convention is
...
```nix
{
  ...
  outputs = { self, nixpkgs, nixdocker, ... }: {
    nixosConfigurations.myhost = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        nixdocker.nixosModules.default   # or nixdocker.systemManagerModules.default
        {
...
| Path | Purpose |
|---|---|
| `flake.nix` | Flake entry point: `nixosModules.nixdocker` / `systemManagerModules.nixdocker` / `.default`, the pure `lib.*`, `checks`, and the deliberately-failing demo `packages` output. |
...
| `modules/nixdocker.nix` | The wiring that is true on every plane: collects every declared object, runs the real generator, installs via `systemd.packages`. |
...
| `modules/ripper.nix` | `nixdocker.ripper` -- the first appliance: an optical-disc ripper as an on-demand job. |
