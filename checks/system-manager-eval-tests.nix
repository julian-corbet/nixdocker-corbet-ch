...
# modules/nixdocker.nix is plane-neutral at all, and it is worth exactly nothing unevaluated -- so
...
let
  ...
  results = [
    ...
    (check "system-manager/units-package-lands-in-the-system-tree"
      (lib.any (p: p.name == "nixdocker-quadlet-system") plain.systemd.packages)
    ...
    (check "system-manager/podman-path-defaults-to-the-distro-binary"
      (plain.nixdocker.podman.path == "/usr/bin/podman")
      "nixdocker.podman.path: ${builtins.toJSON plain.nixdocker.podman.path}")
    ...
    (check "system-manager/pre-activation-assertion-guards-that-path"
      (
        let a = plain.system-manager.preActivationAssertions.nixdocker-podman;
        in a.enable && lib.hasInfix "/usr/bin/podman" a.script
      )
      "preActivationAssertions.nixdocker-podman: ${builtins.toJSON plain.system-manager.preActivationAssertions.nixdocker-podman}")
    ...
    (check "system-manager/ripper-renders-the-same-container-here"
      (
        let text = ripper.nixdocker.containers.ripper.text;
      ...
      "text: ${ripper.nixdocker.containers.ripper.text}")
...
# and still produces a unit that calls the distro's podman.
  ...
  {
    units = ripper.nixdocker.build.systemUnits;
    ...
    ''
      ...
      echo "all $passedCount nixdocker system-manager eval tests passed, and the generated ripper.service was read back from a real generator run" > $out
    ''
