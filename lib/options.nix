...
rec {
      ...
      options = {
        ...
        digest = lib.mkOption {
          ...
          description = ''
            ...
            nixdocker exists at all -- without a digest, Podman (and Quadlet's own `AutoUpdate=`)
        ...
        allowFloatingTag = lib.mkOption {
          ...
          description = ''
            ...
            change to this Nix config. Default `false` on purpose -- THE POINT of nixdocker is that
  ...
  #
  # Build-time synthesis sidesteps this completely, root or rootless alike: nixdocker always runs
  ...
  rootlessOption = lib.mkOption {
    ...
    description = ''
      ...
      Rootful (`null`, the default) is the safe, fully-supported default for nixdocker: one
      ...
      has `users.users.<name>.linger = true` and a subuid/subgid range set up (nixdocker asserts
  ...
  restartIfChangedOption = lib.mkOption {
    ...
    description = ''
      ...
      nixdocker already installs for `wantedBy` -- not in the Quadlet source text, because it is
  ...
  # precisely the 3am discovery nixdocker exists to move to build time; modules/containers.nix
