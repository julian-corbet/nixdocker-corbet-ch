...
# outside its own test suite, and this exact ~90-line technique is core mechanism for nixdocker,
# not a dependency nixdocker should be one `flake.lock` bump away from losing control of.
{ lib }:

{
  ...
  # `podmanPath`, when non-null, is the absolute path the GENERATED unit's `ExecStart=`/
  # `ExecStop=` lines should name -- as opposed to `podman`, the package whose quadlet binary
  # does the generating. On NixOS the two are the same thing and this stays `null`. On a foreign
  # distro driven by system-manager they are NOT: podman there is the distro's own package, and
  # baking a second, nix-built podman into the unit would put two copies of the CLI on one host,
  # sharing one `/var/lib/containers` and free to disagree about its on-disk format.
  #
  # The generator reads exactly one more environment variable than `QUADLET_UNIT_DIRS` for this:
  # `PODMAN`. Verified directly (see docs/gotchas.md) -- the same input generated byte-identical
  # units apart from the three `Exec*=` lines, which named `/usr/bin/podman` instead of the store
  # path. The FLAGS in that ExecStart still come from the generating podman's own vocabulary, so
  # the two want to stay on the same major version; that is a fact about this option, not a
  # reason to avoid it.
  mkQuadletUnitPackage =
    ...
    pkgs.runCommand name
      ...
      } // lib.optionalAttrs (podmanPath != null) { PODMAN = podmanPath; })
      ''
        mkdir -p "${outDir}"
