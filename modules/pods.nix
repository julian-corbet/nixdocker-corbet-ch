# modules/pods.nix
#
# `nixdocker.pods.<name>` -- a shared network/IPC namespace multiple containers join via their own
# `nixdocker.containers.<name>.pod`. Kept deliberately small next to containers.nix: a pod has no
...
in
{
  options.nixdocker.pods = lib.mkOption {
    type = lib.types.attrsOf podType;
    default = { };
    description = "Podman pods declared as build-time-rendered Quadlet units -- a shared network/IPC namespace containers join via `nixdocker.containers.<name>.pod`.";
  };
}
