...
#      reason a foreign-distro host needs `nixdocker.podman.path` instead;
  ...
  config = lib.mkMerge [
    {
      ...
      # which is what makes `nixdocker.podman.path` unnecessary on this plane.
      #
      ...
      nixdocker.podman.package = lib.mkDefault config.virtualisation.podman.package;
    }
    ...
    (lib.mkIf (cfg.build.userUnits != null) {
      ...
      # full reasoning, including why nixdocker never depends on the user GENERATOR running.
      systemd.packages = [ cfg.build.userUnits ];
      systemd.user.services = cfg.build.userOverrides;

      # THE ROOTLESS LINGER REMINDER: nixdocker never manages user accounts (out of scope, the same
      ...
      warnings = lib.mapAttrsToList
        (serviceName: uid: ''
          nixdocker: ${serviceName} installs into uid ${toString uid}'s systemd --user
          ...
          this config -- nixdocker cannot see that state and cannot avoid warning about it here.
        '')
        (lib.filterAttrs
