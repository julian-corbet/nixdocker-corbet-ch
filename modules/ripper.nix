...
# into `nixdocker.containers.<name>` like any other caller would.
#
# WHY AN APPLIANCE BELONGS IN A TRANSLATOR REPO AT ALL. The README's "Boundaries" section already
# says what nixdocker is for: a workload that is single-host BY NATURE -- real local state, a
...
# submodule `nixdocker.containers.<name>.image` uses, so this appliance inherits the repo's own
...
let
  cfg = config.nixdocker.ripper;
  opts = import ../lib/options.nix { inherit lib; };
in
{
  options.nixdocker.ripper = {
    ...
    name = lib.mkOption {
      ...
      default = "ripper";
      description = ''
        The `nixdocker.containers.<name>` this appliance renders into -- which is also the systemd
