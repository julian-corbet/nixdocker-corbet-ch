# NixOS backend for nixdocker baseline packages.
#
...
in
{
  ...
  config = {
    ...
    assertions = lib.optional (unavailable != [ ]) {
      assertion = false;
      message = ''
        nixdocker: ${toString (builtins.length unavailable)} declared baseline package(s) do not resolve in this nixpkgs:
