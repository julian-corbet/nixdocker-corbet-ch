...
# this file talks to podman, systemd, or the generator directly -- see modules/nixdocker.nix for
  ...
  containerType = lib.types.submodule ({ name, config, ... }: {
    options = {
      ...
      pod = lib.mkOption {
        ...
        default = null;
        description = ''
          Name of a `nixdocker.pods.<name>` this container joins (Quadlet `Pod=`). Resolved to a
          ...
          actually declared in `nixdocker.pods`; enforced by this module's own assertion.
        '';
      };
      ...
      network = lib.mkOption {
        ...
        description = ''
          ...
          `nixdocker.networks.<name>` declared alongside this container. `null` (the default)
          ...
          options = {
            cmd = lib.mkOption {
              ...
              description = ''
                ...
                healthcheck nixdocker invented on this service's behalf would be a fake signal;
  ...
  config = {
    assertions = lib.flatten (lib.mapAttrsToList
      (n: c: [
        ...
        {
          assertion = !c.enable || c.pod == null || (config.nixdocker.pods ? ${c.pod});
          message = ''
            nixdocker.containers.${n}.pod points at "${if c.pod == null then "" else c.pod}", which
            is not declared in nixdocker.pods. Quadlet's Pod= only resolves to a real
            BindsTo=/After= pair against a pod unit this same flake actually generates --
            declare nixdocker.pods.${if c.pod == null then "<name>" else c.pod} alongside this
