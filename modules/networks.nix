...
#
# `nixdocker.networks.<name>` -- a named Podman network, pre-created declaratively rather than
...
in
{
  options.nixdocker.networks = lib.mkOption {
    ...
    description = ''
      ...
      `nixdocker.containers.<name>.network = "<name>.network"` gets Quadlet's own automatic
