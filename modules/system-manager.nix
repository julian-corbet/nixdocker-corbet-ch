...
# writes units into without owning the OS underneath. `modules/nixdocker.nix` next to this file
  ...
  config = lib.mkMerge [
    {
      ...
      # containers.
      nixdocker.podman.path = lib.mkDefault "/usr/bin/podman";
    }

    (lib.mkIf enabled {
      system-manager.preActivationAssertions.nixdocker-podman = {
        enable = true;
        script = ''
          if [ ! -x "${cfg.podman.path}" ]; then
            echo "nixdocker: ${cfg.podman.path} is missing or not executable on this host."
            ...
            echo "or point nixdocker.podman.path at wherever it actually lives."
            exit 1
          fi
        '';
      };
      ...
      assertions = [
        {
          ...
          message = ''
            nixdocker: ${lib.concatStringsSep ", " rootlessNames} set rootless.uid, but this is the
            ...
            run that workload on a NixOS host where nixdocker can install a user unit for real.
          '';
        }
      ];
    })
