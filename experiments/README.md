...
outside this Nix config, via `loginctl enable-linger`, which nixdocker cannot observe).
...
kinds (see podman-systemd.unit(5)). nixdocker implements none of them.
...
the most plausible of the three to eventually earn a `nixdocker.images.<name>` module using the
